...
"""Generic, reusable scrollable data table with configurable columns and actions.

Moved from pages/system/components to gui/components for broader reuse.
"""
...
class DataTable(MDBoxLayout):  # pragma: no cover - UI heavy
    ...
    def _open_row_menu(self, row_data, caller):  # pragma: no cover
    ...
    def _invoke_and_close(self, cb):  # pragma: no cover
        for m in list(self._context_menus.values()):
            try:
                m.dismiss()
            except Exception:
