...
class FileCache:
    ...
    def register_serializer(self, extension: str, serializer: Callable[[Any, str], None]):
    ...
    def load_file(self, filepath: str, default_value: Any = None, force_reload: bool = False) -> Any:
        """
        ...
        with self._lock:
            ...
            if (not force_reload and 
                ...
                cached_file.mtime == current_mtime and 
                cached_file.size == current_size and
                not cached_file.dirty):
            ...
            try:
                ...
                print(f"FileCache: Loaded {filepath} ({current_size} bytes)")
                ...
                if default_value is not None:
                    ...
                    self._cache[filepath] = cached_file
                    print(f"FileCache: Error loading {filepath}, using default: {e}")
                    return default_value
                else:
                    raise
    ...
    def _persist_file(self, filepath: str) -> None:
        ...
        try:
            ...
            print(f"FileCache: Saved {filepath} ({new_size} bytes)")
            
        except Exception as e:
            print(f"FileCache: Error saving {filepath}: {e}")
    ...
    def flush_all(self) -> None:
        ...
        with self._lock:
            ...
            if dirty_files:
                print(f"FileCache: Flushed {len(dirty_files)} dirty files")
    ...
    def clear_cache(self, filepath: Optional[str] = None) -> None:
        ...
        with self._lock:
            if filepath:
                ...
                print(f"FileCache: Cleared cache for {filepath}")
            else:
                self._cache.clear()
                print("FileCache: Cleared all cache")
